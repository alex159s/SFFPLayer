...
int sfp_refresh_thread(void *opaque){
	...
	while (thread_exit==0) {
		...
		SDL_Delay(40);//这里的时延有点问题，应该是根据帧率来进行计算的
	}
	//Quit
	SDL_Event event;
    ...
    for (;;) {
        ...
        if (event.type == SFM_REFRESH_EVENT){
            ...
            if (av_read_frame(pFormatCtx, packet) >= 0){
                ...
                finish = clock();
                TRACE("Decode 1 frame,Frame number:%d   Time: %ld\n", videoFrameIndex, finish - start);
                videoFrameIndex++;
            }
            else{
                //Exit Thread
...
void CSFFPlayerDlg::OnBnClickedStart()
{
    ...
    //CPath file = m_url.GetWindowText();
	if(strcmp(url,"")==0){
    ...
    if (pThreadPlay != NULL)
    {
        pThreadPlay->ExitInstance();
        pThreadPlay->Delete();
        pThreadPlay = NULL;
    }
    if (m_bFilePathChanged)
    {
...
bool CSFFPlayerDlg::getFilePathStatus()
{
    return m_bFilePathChanged;
}
